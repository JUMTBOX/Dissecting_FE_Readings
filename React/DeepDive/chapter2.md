...
### 📝2.4 렌더링은 어떻게 일어나는가?
- 렌더링은 브라우저에서도 사용되는 용어이므로 두 가지를 혼동해서는 안 된다.
- 리액트에서의 렌더링이란 리액트 어플리케이션 트리 안에 있는 컴포넌트들이 자신들이 가지고 있는 `props, stat`e 값을 기반으로 UI를 구성하고 이를 바탕으로 어떤 DOM 결과를 브라우저에게 제공할 것인지 계산하는 과정을 의미한다.

**💡리액트의 렌더링이 일어나는 이유**
- 리액트에서 렌더링이 발생하는 시나리오는 다음과 같다.
1. `최초 렌더링`: 사용자가 처음 어플리케이션에 진입하면 당연히 렌더링해야 할 결과물이 필요하다, 브라우저에게 이 정보를 제공하기 위해 최초 렌더링을 수행한다.
2. `리렌더링`: `최초 렌더링`이 발생한 이후로 발생하는 모든 렌더링을 의미한다.
   - 클래스 컴포넌트의 `setState`가 실행되는 경우
   - 클래스 컴포넌트의 `forceUpdate` 실행되는 경우
   - **함수 컴포넌트의 `useState()`훅 반환 값(배열)의 두번째 요소인 `setter`가 실행되는 경우**
   - ```tsx
     import React,{useState} from "react";
     
     export default function App() {
       const [initVal, setInitVal] = useState<string>("초기값"); // <- 여기서 setInitVal 이 `setter`임..
       
       const clickHandler = () => {
         setInitVal(cur => "변환 값");   
       } 
     
       return <div onClick={clickHandler}>{initVal}</div>    
     }
     ```
   - **함수 컴포넌트의 `useReducer()`훅 반환 값(배열)의 두번째 요소인 `dispatch`가 실행되는 경우**
   - **컴포넌트의 `key props`가 변경되는 경우**: `key`는 명시적으로 선언되어 있지 않더라도 모든 컴포넌트에서 사용할 수 있는 특수한 `props`다.
   - ```jsx
        const arr = [1,2,3]; 
        export default function App() {
            return (
                <ul>
                    {arr.map((elem, _index, _origin) => (
                        <li key={elem}>{elem}</li>
                    ))}
                </ul>
            )
   ...
   - 리액트에서 배열에 key를 쓰지 않으면 콘솔에 경고가 출력되기 때문에 key를 유일한 값으로 추가하는 것이 일반적이지만 정작 왜 추가해야 하는지는 모르는 경우가 많다. `왜 key가 필요할까?` 
     ...
     앞서 리액트 파이버 트리 구조를 떠올려보면 해당 트리 구조에서 형제 컴포넌트를 구별하기 위해 각자 sibling이라는 속성값을 사용했다.
     동일한 자식 컴포넌트가 여러 개 있는 구조를 상상해 보자.
     리렌더링이 발생하면 current 트리와 workInProgress 트리 사이에서 어떠한 컴포넌트가 변경이 있었는지 구별해야 하는데, 이 두 트리에서 같은 컴포넌트인지를 구별하는 값이 바로 key다.
     ...
     ```
     ```text
     만약 <Child key={Math.random()} /> 과 같이 렌더링 할 때마다 변하는 임의의 값을 key에 넣는다면 어떻게 될까?
     렌더링이 일어날 때마다 sibling 컴포넌트를 명확히 구분할 수 없으므로 Child는 memo로 선언되었더라도 매번 리렌더링이 일어나게 된다.
     즉, key의 변화는 리렌더링을 야기한다.
     이러한 특징을 이용하면 key를 활용해 강제로 리렌더링을 일으키는 것이 가능해진다.  
     ```
   - **부모컴포넌트가 렌더링될 경우**: 한 가지 주의할 점은, `부모 컴포넌트가 리렌더링된다면 자식 컴포넌트도 무조건 리렌더링이 일어난다`는 것이다.  

**💡리액트의 렌더링 프로세스**
- 렌더링 프로세스가 시작되면 컴포넌트의 루트에서부터 아래쪽으로 내려가면서 업데이트가 필요하다고 지정되어 있는 모든 컴포넌트를 찾는다.
- 만약 여기서 업데이트가 필요하다고 지정된 컴포넌트를 발견하면 함수 컴포넌트는 `FunctionComponent()` 그 자체를 호출한 뒤에, 그 결과물을 저장한다.
- 각 컴포넌트의 렌더링 결과물을 수집한 다음, 리액트의 새로운 트리인 가상 DOM과 비교해 실제 DOM에 반영하기 위한 변경 사항을 수집한다. 이렇게 계산하는 과정을 리액트의 재조정(Reconciliation)이라고 한다.
- 이러한 재조정 과정이 모두 끝나면 모든 변경을 하나의 동기 시퀀스로 DOM에 적용해 변경된 결과물이 보이게 한다. (Commit Phase, 이는 React19까지 동일)
...
- 즉, 컴포넌트를 실행하여 이 결과와 이전 가상 DOM을 비교하는 과정을 거쳐 변경이 필요한 컴포넌트를 체크하는 단계다.
- 여기서 비교하는 것은 크게 세 가지로 `type, props, key`다. 이 세 가지 중 하나라도 변경된 것이 있으면 변경이 필요한 컴포넌트로 지정해 둔다.

**💡커밋 단계(Commit Phase)**
- 렌더 단계의 변경 사항을 실제 DOM에 적용하는 과정을 말한다. 이 단계가 끝나야 비로소 브라우저의 렌더링이 발생한다.
- 리액트가 DOM을 커밋 단계에서 업데이트한다면 이렇게 만들어진 모든 DOM 노드 및 인스턴스를 가리키도록 리액트 내부의 참조를 업데이트한다. 그 다음 함수 컴포넌트에서는 `useLayoutEffect`훅을 호출한다.
- 중요한 사실은 `리액트의 렌더링이 일어난다고 해서 무조건 DOM 업데이트가 일어나는 것은 아니라는 것이다.`
- 렌더링을 수행했으나 커밋 단계까지 갈 필요가 없다면, 즉 아무런 변경 사항이 감지되지 않는다면 이 커밋 단계는 생략될 수 있다.
- 즉, 리액트의 렌더링은 꼭 가시적인 변경이 일어나지 않아도 발생할 수 있다.
- 컴포넌트를 렌더링하는 작업은 별도로 렌더링을 피하기 위한 조치가 되어 있지 않는 한 하위 모든 컴포넌에 영향을 미친다.
<br/>그리고 부모가 변경되었다면 props가 변경되었는지와 관계없이 무조건 자식 컴포넌트도 렌더링 된다. `*주의* 리렌더가 아님`
<br/>다만 자식 컴포넌트가 `React.memo()`로 감싸져 있다면 현재 `props(currentProps)`와 이전 `props(prevProps)`에 대해 
<br/>`shallowEqual(prevProps, currentProps)`로 얕은 비교를 거친 후 두 props가 동일하다면 해당 컴포넌트는 렌더링을 생략하고 이전 결과를 재사용한다.

**💡컴포넌트와 함수의 무거운 연산을 기억해 두는 메모이제이션**
- 의견1: `메모이제이션은 비용이 드는 작업이기 때문에 신중하게 사용하여야 한다.`
  - 값을 비교하고 렌더링 또는 재계산이 필요한지 확인하는 작업 & 이전의 결과물을 저장하고 그것을 다시 꺼내오는 작업
  - `메모이제이션` 항상 어느 정도의 trade-off가 있는 기법이라고 보는 것이 옳다.
- 의견2: `섣부른 최적화`를 했을 때의 잠재적 비용보다 최적화가 필요한 곳에 적용하지 않았을 때의 잠재적 비용이 훨씬 높기 때문에 일단 모든 곳에 메모이제이션을 적용하고 살펴보는 것이 낫다.  


## 더 알아보기
<h3>
  다른 문서 링크 : <a href="./Extra/AsynchronousTaskOfReact.md"> 💥 React의 비동기 작업은 어떻게 구현되어 있을까? </a>
</h3>

### <a> 💥 React19 에서의 JSX 트랜스파일링 </a>
...
```
- babel 설정을 고의로 ```runtime:"classic"``` 으로 바꾸면 예전처럼 ```React.createElement(...)```로도 변환할 수는 있으나 React 19 권장 사항에 어긋남

### <a>💥 `React.memo` 사용하면 만사 OK?</a>
- 부모가 리렌더링될 때, 자식이 `React.memo`일 때의 비용은 대략
  - `shallowEqual(prevProps, currentProps)` 한 번
  - prop 개수를 k라고 하면, `O(k)`
  - 각 필드는 `Object.is(prev, current)` 한 번씩 
  - props가 다르게 나왔다면 결국 자식 컴포넌트 함수도 호출
- `즉, 비교 비용 + 렌더 비용 둘 다 지출`
- 반대로, `React.memo` 안 쓰는 일반 자식은: 그냥 자식 함수 한 번 호출 (렌더 비용만 있음)

- **props가 자주 바뀌는 자식이라면**
<br> → `React.memo`를 써도 매번 `shallowEqual + 렌더 둘 다 실행 ⇒ Dog손해`

- **props가 자주 안 바뀌고, 자식 렌더 비용이 비싸면**
<br> → 한 번의 `shallowEqual` 비용으로 자식 전체 렌더를 여러 번 스킵할 수 있으니 이득

**손해인 경우**
1. 아주 단순한 자식 컴포넌트
2. props가 거의 매번 바뀌는 경우`(정확하게는 props 객체의 1depth 필드들의 값이 항상 바뀌는 경우)`
3. 부모가 자주 리렌더링되는 경우
- **이 셋이 겹치는 케이스라면 memo를 지양하자...**
