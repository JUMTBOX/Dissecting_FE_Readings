...
> - **`그렇다면 그냥 함수 컴포넌트의 몸체에서 실행되는 코드와의 차이점은 무엇인가?`**
> <br/> ->  `useEffect` 는 클라이언트 사이드에서 실행되는 것을 보장해준다. `useEffect`에서는 window 전역 객체에 접근하는 코드를 사용해도 무방하다.
> <br/>
...
```jsx
/* [사용법] */
 useCallback(() => { /* memorize */ },[dependency])
```
...
> **`렌더링에 영향을 미치지 않는 고정값이라는 목적을 위해서라면, 함수 컴포넌트의 선언 밖에서 변수를 선언하여 관리하는 것과 차이가 없지 않을까?`**
><br/> -> 컴포넌트 렌더링 이전에도 변수가 존재하게되어 메모리 낭비가 발생한다.
...
### <a> 💥 `useEffect` 콜백의 내부에서 참조하는 모든 `reactive value`는 의존성 배열에 들어가야한다? </a>